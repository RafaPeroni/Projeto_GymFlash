...
if(isset($_POST['cadusu'])){
 ...
 // verificação de cadastro do email de academia já cadastrado
        $veri_acad = $conn->prepare("SELECT * FROM CadastroAcademia WHERE Email_academia like '$email_usu' ");
        $veri_acad->execute();
        $result2 = $veri_acad->fetch(PDO::FETCH_ASSOC);
        ...
        $stmt->execute();
        function_alert('Cadastro realizado com sucesso!');
           
        
        die("Clique aki para retornar:.  <p><a href=\"./index.php\">RETORNAR</a></p>");
